...
ms_peak_statistics <- function() {
  ...
  install_and_load_required_packages(c("rattle", "phia", "MASS", "ggplot2", "lawstat", "coin", "multcomp", "agricolae", "tcltk", "Hmisc", "parallel"), update_packages = FALSE, print_messages = TRUE) # "Rcmdr", "RcmdrPlugin.coin"
  ...
  check_for_updates_function <- function() {
    ...
    if (there_is_internet == TRUE) {
      try({
        ...
        online_version_YYYYMMDDVV <- numeric_version(online_version_number)
        ### Compare with the local version
        local_version_YYYYMMDDVV <- numeric_version(R_script_version)
        ### Check the versions (from the Year to the Day)
        if (local_version_YYYYMMDDVV < online_version_YYYYMMDDVV) {
  ...
  file_import_function <- function() {
    ...
    if (input_file != "") {
      ...
      # Discriminant column
      tkmessageBox(title = "Discriminant/Splitting feature", message = "Select the discriminant/splitting feature", icon = "info")
      ...
      patient_id_attribute <- select.list(c(non_signals, "NONE"), title = "Patient ID attribute", preselect = "Sample")
      # Raise the main window
      tkraise(window)
      ...
      if (isTRUE(correlation_analysis)) {
        ...
        for (cl in 1:length(class_list)) {
          ...
          if (nrow(data_frame_correlation) >= minimum_number_of_patients) {
            ### If there are non-signals...
            if (length(non_signals_for_correlation_analysis) > 0) {
              ### For each non-signal...
              ...
              print("The correlation will not be performed!")
            }
          } else {
            ### Correlation not possible
      ...
      if (isTRUE(two_level_effect_analysis)) {
        ...
        for (l in 1:length(combination_vector)) {
          if (number_of_levels > 2) {
            ...
            combination_name <- paste(names(combination_vector[l]), levels(as.factor(data_column))[1], "vs", levels(as.factor(data_column))[2])
          }
